...
fn update_screen_effect(
    ...
    for (screen_effect, mut material_handle) in &mut query {
...
fn setup(
    ...
    // Scifi corners (back-left)
    commands.spawn(PbrBundle {
        mesh: meshes.add(Mesh::from(Cuboid::new(1.0, 4.0, 1.0))),
        material: materials.add(StandardMaterial {
            base_color_texture: Some(asset_server.load("textures/wall_paneling.png")),
            perceptual_roughness: 0.8,
            metallic: 0.1,
        ...
        transform: Transform::from_xyz(-4.5, 2.0, -4.5).with_rotation(Quat::from_rotation_y(std::f32::consts::FRAC_PI_4)),
        ..default()
    })
    .insert(Collider::cuboid(0.5, 2.0, 0.5));

    // Scifi corners (back-right)
    commands.spawn(PbrBundle {
        mesh: meshes.add(Mesh::from(Cuboid::new(1.0, 4.0, 1.0))),
        material: materials.add(StandardMaterial {
            base_color_texture: Some(asset_server.load("textures/wall_paneling.png")),
